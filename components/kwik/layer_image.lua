...
--
local createInfinityImage(self, sceneGroup, layer)
  local layer_2 = display.newImageRect( _K.imgDir..self.imagePath, _K.systemDir, self.imageWidth, self.imageHeight)
  ...
  layer_2.anchorX = 0
  layer_2.anchorY = 0;
  _K.repositionAnchor(layer_2, 0,0)
  --
...
function _M:myNewImage(UI)
  ...
  local layer = display.newImageRect( _K.imgDir..self.imagePath, _K.systemDir, self.imageWidth, self.imageHeight)
  -- layer = newImageRect({{bn}}, imageWidth, imageHeight )
  if layer == nil then return end
  layer.imagePath = self.imagePath
  layer.x = self.mX
  layer.y = self.mY
  layer.alpha = self.ooriAlpha
...
function _M:comicImage(UI)
  ...
  local options = {
   ...
   local sheet = graphics.newImageSheet(_K.imgDir..self.imagePath, _K.systemDir, options )
   for i=1, #self.layerSet} do
     local target = self.layerSet[i]
